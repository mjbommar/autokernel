...
_LAPTOP = WorkloadProfileSpec(
    ...
    recipes=[
        ...
        WorkloadRecipe("PM_RUNTIME",             "y", "power",
            "Per-device runtime PM; biggest single battery win for buses/devices.",
            "kernel.org/admin-guide/pm"),
        WorkloadRecipe("HIBERNATION",            "y", "power",
            "Suspend-to-disk for laptops with limited battery.",
            "kernel.org"),
        WorkloadRecipe("ACPI_AC",                "y", "power",
            "AC-status events.",
            "Gentoo PM guide"),
        WorkloadRecipe("ACPI_BATTERY",           "y", "power",
            "Battery-status events.",
            "Gentoo PM guide"),
        WorkloadRecipe("ACPI_FAN",               "y", "power",
            "Fan/thermal control.",
            "Gentoo PM guide"),
        WorkloadRecipe("ACPI_THERMAL",           "y", "power",
            "Required for thermal management.",
            "kernel.org"),
        WorkloadRecipe("ACPI_DOCK",              "y", "power",
            "Dock/undock events.",
            "ThinkWiki"),
        WorkloadRecipe("INTEL_PSTATE",           "y", "power",
            "Native HWP driver for Skylake+ Intel; far better than acpi-cpufreq.",
            "kernel.org/admin-guide/pm/intel_pstate"),
        WorkloadRecipe("X86_AMD_PSTATE",         "y", "power",
            "CPPC-based driver for Zen2+ AMD with EPP.",
            "kernel.org/admin-guide/pm/amd-pstate"),
        WorkloadRecipe("CPU_FREQ_DEFAULT_GOV_SCHEDUTIL", "y", "power",
            "schedutil is now the recommended default — sched-integrated, energy-aware.",
            "ArchWiki cpufreq"),
        WorkloadRecipe("INTEL_IDLE",             "y", "power",
            "Deeper C-states than acpi_idle.",
            "kernel.org"),
        WorkloadRecipe("PCIEASPM",               "y", "power",
            "Aggressive PCIe runtime PM — biggest single battery-life win.",
            "Ubuntu,Arch PM"),
        WorkloadRecipe("USB_AUTOSUSPEND",        "y", "power",
            "USB device runtime PM.",
            "ArchWiki Power management"),
        WorkloadRecipe("ACPI_PLATFORM_PROFILE",  "y", "power",
            "Modern Performance/Balanced/Quiet platform hooks.",
            "kernel.org"),
        WorkloadRecipe("BACKLIGHT_CLASS_DEVICE", "y", "compat",
            "Brightness keys.",
            "kernel.org"),
        WorkloadRecipe("EFI_RUNTIME_WRAPPERS",   "y", "compat",
            "Modern UEFI laptop expects these.",
            "Fedora kernel.spec"),
        WorkloadRecipe("THINKPAD_ACPI",          "m", "compat",
...
_SERVER = WorkloadProfileSpec(
    ...
    recipes=[
        WorkloadRecipe("PREEMPT_NONE",           "y", "perf",
            "'Traditional Linux preemption model, geared towards throughput' — recommended for server/scientific.",
            "LKDDB"),
        WorkloadRecipe("HZ_100",                 "y", "perf",
            "RHEL/SUSE server tick rate; minimum interrupt overhead.",
            "RHEL low-latency PDF"),
        WorkloadRecipe("NO_HZ_IDLE",             "y", "perf",
            "Idle ticks gone — saves CPU and helps virt neighbors.",
            "kernel.org"),
        WorkloadRecipe("NUMA",                   "y", "perf",
            "Required on multi-socket.",
            "RHEL"),
        WorkloadRecipe("NUMA_BALANCING",         "y", "perf",
            "Auto page-migration to local node — 'most important task' for multi-socket.",
            "SLES tuning"),
        WorkloadRecipe("NUMA_BALANCING_DEFAULT_ENABLED", "y", "perf",
            "Auto-on at boot.",
            "kernel.org"),
        WorkloadRecipe("SCHED_MC",               "y", "perf",
            "Multi-core sched topology awareness.",
            "kernel.org"),
        WorkloadRecipe("SCHED_SMT",              "y", "perf",
            "SMT-aware sched.",
            "kernel.org"),
        WorkloadRecipe("SCHED_CLUSTER",          "y", "perf",
            "Cluster-aware (shared L2 on Zen5/Alder Lake/big.LITTLE).",
            "Phoronix cluster sched"),
        WorkloadRecipe("HUGETLBFS",              "y", "perf",
            "Explicit huge pages for DBs / DPDK.",
            "RHEL"),
        WorkloadRecipe("TCP_CONG_BBR",           "y", "perf",
            "Standard for cloud-edge workloads.",
            "Cloudflare,Google"),
        WorkloadRecipe("DEFAULT_BBR",            "y", "perf",
            "Make BBR the default congestion control.",
            "arXiv 2510.22461"),
        WorkloadRecipe("NET_SCH_FQ",             "y", "perf",
            "FQ qdisc for BBR pacing.",
            "kernel.org"),
        WorkloadRecipe("NET_SCH_FQ_CODEL",       "y", "perf",
            "fq_codel default qdisc.",
            "kernel.org"),
        WorkloadRecipe("RPS",                    "y", "perf",
            "Receive packet steering across cores.",
            "Linux net"),
        WorkloadRecipe("RFS_ACCEL",              "y", "perf",
            "RFS accelerator for steady flows.",
            "Linux net"),
        WorkloadRecipe("XPS",                    "y", "perf",
            "Transmit packet steering.",
            "Linux net"),
        WorkloadRecipe("BPF_SYSCALL",            "y", "perf",
            "eBPF data plane (Cilium, Katran, etc.).",
            "Cilium"),
        WorkloadRecipe("BPF_JIT",                "y", "perf",
            "JIT vs interpreter — large speedup for any BPF.",
            "iovisor"),
        WorkloadRecipe("BPF_JIT_DEFAULT_ON",     "y", "perf",
            "Auto-JIT instead of needing sysctl.",
            "Cloudflare"),
        WorkloadRecipe("XDP_SOCKETS",            "y", "perf",
            "AF_XDP for kernel-bypass dataplane.",
            "Cilium"),
        WorkloadRecipe("PCI_IOV",                "y", "perf",
            "SR-IOV for accelerator/NIC pass-through.",
            "kernel.org"),
        WorkloadRecipe("VFIO",                   "m", "perf",
            "VFIO for device assignment to userspace/VMs.",
            "kernel.org"),
        WorkloadRecipe("IOMMU_DEFAULT_PASSTHROUGH", "y", "perf",
            "Skip IOMMU mappings for trusted DMA — measurable throughput win.",
            "LWN"),
        WorkloadRecipe("PSI",                    "y", "perf",
            "PSI metrics for autoscalers.",
            "kernel.org"),
        WorkloadRecipe("CGROUP_BPF",             "y", "perf",
            "Containers / k8s.",
            "Ubuntu generic"),
        WorkloadRecipe("HIBERNATION",            "n", "surface",
            "Servers don't sleep; trims attack surface and image size.",
            "RHEL"),
        WorkloadRecipe("SUSPEND",                "n", "surface",
            "Servers don't sleep.",
            "RHEL"),
        WorkloadRecipe("SOUND",                  "n", "surface",
            "Drop hot-plug consumer surface.",
            "RHEL"),
        WorkloadRecipe("BT",                     "n", "surface",
            "No Bluetooth on a datacenter box.",
            "RHEL"),
        WorkloadRecipe("INPUT_JOYDEV",           "n", "surface",
            "No game controllers in production.",
            "RHEL"),
    ],
)
...
_VM_GUEST = WorkloadProfileSpec(
    ...
    recipes=[
        WorkloadRecipe("HYPERVISOR_GUEST",       "y", "compat",
            "Master switch.",
            "linux/kernel/configs/kvm_guest.config"),
        WorkloadRecipe("PARAVIRT",               "y", "perf",
            "Paravirt hooks; PV-spinlocks materially reduce lock-holder preemption stalls.",
            "kvm_guest.config"),
        WorkloadRecipe("PARAVIRT_SPINLOCKS",     "y", "perf",
            "Required for KVM/Xen guests to avoid lock-holder preemption.",
            "Clovertrail KVM PV spinlocks"),
        WorkloadRecipe("KVM_GUEST",              "y", "perf",
            "kvmclock, async PF, PV TLB flush, PV IPI — sizable wins inside KVM.",
            "LWN"),
        WorkloadRecipe("XEN",                    "y", "perf",
            "Xen guest support — PVH/PV.",
            "kvm_guest.config"),
        WorkloadRecipe("HYPERV",                 "y", "perf",
            "Hyper-V enlightenments give ~20% single-core / 11% multi-core.",
            "Red Hat optimizing Windows VMs"),
        WorkloadRecipe("VIRTIO_PCI",             "y", "compat",
            "Virtio PCI bus.",
            "kvm_guest.config"),
        WorkloadRecipe("VIRTIO_BLK",             "y", "compat",
            "Built-in (not modular) so initrd is small/optional.",
            "kvm_guest.config"),
        WorkloadRecipe("VIRTIO_NET",             "y", "compat",
            "Built-in. Standard guest NIC.",
            "kvm_guest.config"),
        WorkloadRecipe("VIRTIO_CONSOLE",         "y", "compat",
            "Console + agent transport.",
            "libvirt wiki"),
        WorkloadRecipe("VIRTIO_BALLOON",         "y", "perf",
            "Ballooning for memory hot-plug.",
            "libvirt wiki"),
        WorkloadRecipe("VIRTIO_INPUT",           "y", "compat",
            "QEMU virtio-tablet.",
            "libvirt wiki"),
        WorkloadRecipe("VIRTIO_FS",              "m", "perf",
            "Lightweight host-shared FS.",
            "libvirt wiki"),
        WorkloadRecipe("VIRTIO_VSOCK",           "m", "compat",
            "vsock for agents.",
            "kernel.org"),
        WorkloadRecipe("HW_RANDOM_VIRTIO",       "y", "perf",
            "Avoids boot-time entropy starvation; cloud hosts expose this.",
            "Debian BoottimeEntropyStarvation"),
        WorkloadRecipe("RANDOM_TRUST_CPU",       "y", "perf",
            "Trust RDRAND — gets crng_init done before userspace.",
            "kernel.org"),
        WorkloadRecipe("RANDOM_TRUST_BOOTLOADER","y", "perf",
            "Trust hypervisor-supplied seed.",
            "kernel.org"),
        WorkloadRecipe("HZ_250",                 "y", "perf",
        ...
        WorkloadRecipe("NO_HZ_IDLE",             "y", "perf",
            ...
            "kernel.org"),
        WorkloadRecipe("PREEMPT_VOLUNTARY",      "y", "perf",
        ...
        WorkloadRecipe("SUSPEND",                "n", "surface",
            "Same as hibernation.",
            "Fedora cloud"),
        WorkloadRecipe("THERMAL",                "n", "surface",
            "No thermal sensors in a VM.",
            "RHEL Atomic"),
        WorkloadRecipe("CPU_FREQ",               "n", "surface",
            "Hypervisor scales frequency, not the guest.",
            "RHEL Atomic"),
        WorkloadRecipe("DRM",                    "n", "surface",
        ...
        WorkloadRecipe("BT",                     "n", "surface",
            "Drop Bluetooth.",
            "Alpine virt"),
        WorkloadRecipe("X86_X2APIC",             "y", "perf",
...
_REALTIME = WorkloadProfileSpec(
    ...
    recipes=[
        WorkloadRecipe("PREEMPT_RT",             "y", "perf",
        ...
        WorkloadRecipe("HZ_1000",                "y", "perf",
            ...
            "Ubuntu lowlatency"),
        WorkloadRecipe("HIGH_RES_TIMERS",        "y", "perf",
            "hrtimers are non-negotiable.",
            "kernel.org"),
        WorkloadRecipe("NO_HZ_FULL",             "y", "perf",
            "Eliminate scheduling-clock interrupts on isolated CPUs.",
            "kernel.org"),
        WorkloadRecipe("RCU_NOCB_CPU",           "y", "perf",
            "Offload RCU callbacks; required to actually exit ticks under nohz_full.",
            "linuxfoundation RT wiki"),
        WorkloadRecipe("RCU_NOCB_CPU_DEFAULT_ALL", "y", "perf",
            "Offload all CPUs' callbacks by default.",
            "linuxfoundation RT wiki"),
        WorkloadRecipe("RCU_BOOST",              "y", "perf",
            "Avoid RCU-reader-induced grace-period blocking.",
            "linuxfoundation RT"),
        WorkloadRecipe("IRQ_FORCED_THREADING_DEFAULT", "y", "perf",
            "Threaded IRQs by default — schedulable, priority-able.",
            "bootlin RT slides"),
        WorkloadRecipe("DEBUG_PREEMPT",          "n", "perf",
            "Critical: disable debug options that introduce latency.",
            "Ubuntu RT"),
        WorkloadRecipe("DEBUG_LOCKDEP",          "n", "perf",
            "Same.",
            "Ubuntu RT"),
        WorkloadRecipe("DEBUG_OBJECTS",          "n", "perf",
            "Same.",
            "Ubuntu RT"),
        WorkloadRecipe("SLUB_DEBUG",             "n", "perf",
            "Same.",
            "Ubuntu RT"),
        WorkloadRecipe("CPU_FREQ_DEFAULT_GOV_PERFORMANCE", "y", "perf",
            "Avoid frequency-transition latency.",
            "Ubuntu RT"),
        WorkloadRecipe("TRANSPARENT_HUGEPAGE",   "n", "perf",
            "THP coalescing/khugepaged causes stalls — RT can't tolerate.",
            "Ubuntu RT,RHEL"),
        WorkloadRecipe("HUGETLBFS",              "y", "perf",
            "Explicit huge pages OK; transparent ones not.",
            "kernel.org"),
        WorkloadRecipe("NET_RX_BUSY_POLL",       "y", "perf",
            "Low-latency NIC RX.",
            "Ubuntu RT"),
        WorkloadRecipe("BPF_JIT",                "y", "perf",
            "Avoid interpreter latency for XDP/cgroup-bpf paths.",
            "Cilium"),
        WorkloadRecipe("BPF_JIT_ALWAYS_ON",      "y", "perf",
            "Foreclose interpreter (Spectre v2 hardening + smaller text).",
            "LKDDB"),
        WorkloadRecipe("PCIEASPM",               "n", "perf",
            "ASPM transition latency kills tail.",
            "Ubuntu RT"),
        WorkloadRecipe("VIRT_CPU_ACCOUNTING_GEN","y", "perf",
...
_EMBEDDED = WorkloadProfileSpec(
    ...
    recipes=[
        ...
        WorkloadRecipe("SUSPEND",                "n", "surface",
            ...
            "Yocto"),
        WorkloadRecipe("SQUASHFS",               "y", "compat",
            "Standard read-only embedded rootfs.",
            "Yocto"),
        WorkloadRecipe("SQUASHFS_XZ",            "y", "compat",
            "XZ for squashfs.",
            "Yocto"),
        WorkloadRecipe("KERNEL_ZSTD",            "y", "perf",
            "Self-decompressing kernel, smaller flash footprint.",
            "Yocto kernel-size"),
        WorkloadRecipe("DEBUG_INFO_NONE",        "y", "surface",
            "Don't ship debug bloat.",
            "Yocto"),
        WorkloadRecipe("FTRACE",                 "n", "surface",
            "Drop ftrace surface.",
            "Yocto"),
    ],
)
