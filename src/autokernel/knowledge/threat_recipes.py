...
_BALANCED = ThreatProfileSpec(
    ...
    recipes=[
        ...
        ThreatRecipe(
            "MITIGATION_PAGE_TABLE_ISOLATION",
            "y",
            "Meltdown KPTI; cost 5-30% syscalls. Mandatory.",
            "kernel.org PTI",
        ),
        ThreatRecipe(
        ...
        ThreatRecipe("RANDOMIZE_BASE", "y", "KASLR. Negligible runtime cost.", "KSPP"),
        ThreatRecipe(
            "RANDOMIZE_MEMORY",
            "y",
            "physmap/vmalloc/vmemmap base randomization.",
            "KSPP",
        ),
        # Heap/stack
        ThreatRecipe(
            "FORTIFY_SOURCE", "y", "str/mem overflow checks. ~zero cost.", "KSPP"
        ),
        ThreatRecipe(
            "STACKPROTECTOR_STRONG", "y", "Stack canaries; <1% overhead.", "KSPP"
        ),
        ThreatRecipe(
            "VMAP_STACK",
            "y",
            "Guard pages + vmap'd stacks; defangs stack overruns.",
            "KSPP",
        ),
        ThreatRecipe(
            "RANDOMIZE_KSTACK_OFFSET_DEFAULT",
            "y",
            "Per-syscall stack-offset rand. <1%.",
            "KSPP",
        ),
        ThreatRecipe(
        ...
        ThreatRecipe(
            "INIT_STACK_ALL_ZERO",
            "y",
            "-ftrivial-auto-var-init=zero. Negligible.",
            "KSPP",
        ),
        # Slab
        ThreatRecipe(
            "SLAB_FREELIST_RANDOM", "y", "Per-slab freelist randomization.", "KSPP"
        ),
        ThreatRecipe(
            "SLAB_FREELIST_HARDENED", "y", "XOR-obfuscated freelist pointers.", "KSPP"
        ),
        ThreatRecipe(
            "RANDOM_KMALLOC_CACHES", "y", "16 caller-keyed kmalloc caches.", "KSPP"
        ),
        ThreatRecipe(
            "SHUFFLE_PAGE_ALLOCATOR", "y", "Buddy allocator randomization.", "KSPP"
        ),
        ThreatRecipe(
            "HARDENED_USERCOPY", "y", "Bounds-check copy_to/from_user.", "KSPP"
        ),
        ThreatRecipe(
            "BUG_ON_DATA_CORRUPTION",
            "y",
            "Panic on detected list/refcount corruption.",
            "KSPP",
        ),
        # LSMs
        ThreatRecipe("SECURITY_YAMA", "y", "ptrace_scope; near-free.", "KSPP"),
        ThreatRecipe("SECURITY_LANDLOCK", "y", "Unprivileged sandboxing API.", "KSPP"),
        ThreatRecipe(
            "SECURITY_LOCKDOWN_LSM",
            "y",
        ...
        ThreatRecipe(
            ...
            "y",
            "Apply lockdown before other LSMs init.",
            "KSPP",
        ),
        ThreatRecipe(
            "SECURITY_DMESG_RESTRICT",
            "y",
            "Default kernel.dmesg_restrict=1; hides kptrs.",
            "KSPP",
        ),
        # Modules
        ThreatRecipe("MODULE_SIG", "y", "Signed-module support.", "KSPP"),
        ThreatRecipe("STRICT_MODULE_RWX", "y", "Enforce W^X on module text.", "KSPP"),
        ThreatRecipe(
            "MODULE_SIG_SHA512", "y", "Avoid SHA-1 for module signing.", "KSPP"
        ),
        # Surface
        ThreatRecipe(
            "STRICT_DEVMEM", "y", "Restrict /dev/mem to devmem regions.", "KSPP"
        ),
        ThreatRecipe("IO_STRICT_DEVMEM", "y", "Block ioport overlap.", "KSPP"),
        ThreatRecipe("LEGACY_TIOCSTI", "n", "Block TTY-injection escapes.", "KSPP"),
        ThreatRecipe(
        ...
        # Numeric
        ThreatRecipe(
            "DEFAULT_MMAP_MIN_ADDR", "65536", "Block NULL-deref-to-mmap.", "KSPP"
        ),
    ],
)
...
_PARANOID = ThreatProfileSpec(
    ...
    recipes=[
        ...
        ThreatRecipe(
            ...
            "raises exploit-dev cost significantly.",
            "KSPP",
        ),
        ThreatRecipe(
            "ZERO_CALL_USED_REGS",
        ...
        ThreatRecipe(
            "UBSAN_BOUNDS", "y", "Array bounds checks; trap on violation.", "KSPP"
        ),
        ThreatRecipe(
            "UBSAN_TRAP", "y", "Hardware trap on UBSAN finding (not just log).", "KSPP"
        ),
        ThreatRecipe(
            "KFENCE", "y", "Sampling UAF/OOB detector. ~free at sample=100ms.", "KSPP"
        ),
        ThreatRecipe(
        ...
        ThreatRecipe("MODULE_SIG_FORCE", "y", "Refuse unsigned modules.", "KSPP"),
        ThreatRecipe("MODULE_SIG_ALL", "y", "Sign during build.", "KSPP"),
        ThreatRecipe(
            "IMA", "y", "File hashing on access; per-open hashing cost.", "Red Hat IMA"
        ),
        ThreatRecipe(
            "IMA_APPRAISE",
            "y",
            "Enforce signatures against measurements.",
            "Gentoo IMA",
        ),
        ThreatRecipe(
            "EVM", "y", "Protects xattrs (security.ima); pairs with IMA.", "Gentoo EVM"
        ),
        ThreatRecipe(
            "KEXEC_SIG_FORCE", "y", "If kexec on, require signed kernels.", "KSPP"
        ),
        # Surface reduction (paranoid only)
        ThreatRecipe(
            "X86_X32_ABI", "n", "Frequent CVE source; rarely used. Disable.", "KSPP"
        ),
        ThreatRecipe(
            "IA32_EMULATION",
            "n",
        ...
        ThreatRecipe("MODIFY_LDT_SYSCALL", "n", "Wine/dosemu only.", "KSPP"),
        ThreatRecipe("X86_MSR", "n", "/dev/cpu/*/msr.", "KSPP"),
        ThreatRecipe(
            "PROC_KCORE", "n", "Crash-dump utility; otherwise leak source.", "KSPP"
        ),
        ThreatRecipe("DEVMEM", "n", "/dev/mem; physmem read/write.", "KSPP"),
        ThreatRecipe(
            "LEGACY_VSYSCALL_NONE",
            "y",
            "Kill vsyscall entirely (modern glibc).",
            "KSPP",
        ),
        ThreatRecipe(
            "KEXEC", "n", "Replace running kernel; disable on desktop.", "KSPP"
        ),
        ThreatRecipe("HIBERNATION", "n", "Image-write attack vector.", "KSPP"),
        ThreatRecipe(
            "ACPI_CUSTOM_METHOD", "n", "Direct physmem write via AML.", "KSPP"
        ),
        ThreatRecipe("COMPAT_BRK", "n", "Disables brk-ASLR.", "KSPP"),
        ThreatRecipe("COMPAT_VDSO", "n", "Defeats vDSO ASLR.", "KSPP"),
    ],
)
...
threat_recipes: dict[str, ThreatProfileSpec] = {
    spec.threat: spec for spec in (_PERMISSIVE, _BALANCED, _PARANOID)
}
