...
_RULES_DEF: list[tuple[str, list[str]]] = [
    ...
    ("cpufreq", [r"^CPU_FREQ", r"^X86_INTEL_PSTATE", r"^X86_AMD_PSTATE", r"^X86_AMD_FREQ",
                 r"^X86_AMD_PLATFORM_DEVICE$", r"^X86_POWERNOW"]),
    ("cpuidle", [r"^CPU_IDLE", r"^INTEL_IDLE", r"^ACPI_PROCESSOR_IDLE"]),
    ("microcode", [r"^MICROCODE"]),

    # ── arch fundamentals ────────────────────────────────────────────────
    ("arch", [r"^X86($|_)", r"^64BIT$", r"^SMP$", r"^ARM($|64$|_)", r"^RISCV", r"^PPC", r"^MIPS"]),
    ("arch", [r"^IA32_EMULATION$", r"^COMPAT_", r"^X32_ABI$"]),

    # ── boot / init ──────────────────────────────────────────────────────
    ("boot", [r"^EFI($|_)", r"^BLK_DEV_INITRD$", r"^RD_(GZIP|LZ4|ZSTD|XZ|LZMA|LZO|BZIP2)$", r"^BOOT", r"^CMDLINE", r"^DMI"]),

    # ── core kernel infrastructure ───────────────────────────────────────
    ("core", [r"^PRINTK", r"^TTY$", r"^VT($|_)", r"^UNIX98_PTYS$", r"^DEVTMPFS", r"^TMPFS", r"^PROC_FS$",
              r"^SYSFS$", r"^FUTEX", r"^EPOLL", r"^SIGNALFD", r"^TIMERFD", r"^EVENTFD", r"^BLOCK$",
              r"^MULTIUSER$", r"^MODULES$", r"^MODULE_", r"^MMU$", r"^SWAP$", r"^SHMEM$",
              r"^HW_RANDOM($|_)", r"^SYSVIPC", r"^POSIX_MQUEUE$", r"^NAMESPACES$", r"^USER_NS$",
              r"^PID_NS$", r"^IPC_NS$", r"^UTS_NS$", r"^NET_NS$", r"^TIME_NS$", r"^CHECKPOINT_RESTORE$"]),

    # ── cgroups (their own bucket; systemd-critical) ─────────────────────
    ("cgroups", [r"^CGROUP", r"^MEMCG"]),

    # ── memory management ────────────────────────────────────────────────
    ("mm", [r"^TRANSPARENT_HUGEPAGE", r"^HUGETLB", r"^KSM$", r"^MEMORY_HOTPLUG", r"^ZSWAP", r"^ZRAM",
            r"^SPARSEMEM", r"^FLATMEM", r"^NUMA", r"^MEMORY_FAILURE", r"^CMA"]),

    # ── crypto / security infrastructure ─────────────────────────────────
    ("crypto", [r"^CRYPTO"]),
    ("security", [r"^SECURITY", r"^SELINUX", r"^APPARMOR", r"^SMACK", r"^LSM", r"^INTEGRITY",
                  r"^IMA($|_)", r"^EVM", r"^AUDIT", r"^FORTIFY_SOURCE", r"^STACKPROTECTOR"]),
    ("kasan", [r"^KASAN", r"^KMSAN", r"^UBSAN", r"^KFENCE", r"^KCSAN"]),

    # ── debug / tracing ──────────────────────────────────────────────────
    ("debug", [r"^DEBUG", r"^FTRACE", r"^TRACE", r"^DYNAMIC_DEBUG", r"^GDB_SCRIPTS", r"^KPROBES",
               r"^FUNCTION_TRACER", r"^STACKTRACE", r"^LOCK_STAT", r"^LOCKDEP", r"^PROVE_",
               r"^FAULT_INJECTION", r"^MAGIC_SYSRQ"]),
    ...
    ("pm", [r"^PM($|_)", r"^SUSPEND", r"^HIBERNATION", r"^PM_AUTOSLEEP$", r"^PM_WAKELOCKS$"]),
    ("acpi", [r"^ACPI"]),

    # ── graphics / display ───────────────────────────────────────────────
    ("gpu", [r"^DRM", r"^FB($|_)", r"^AGP", r"^I915$", r"^VGA_ARB", r"^FRAMEBUFFER_CONSOLE",
             r"^BACKLIGHT_CLASS_DEVICE$", r"^LCD_CLASS_DEVICE$"]),

    # ── sound ────────────────────────────────────────────────────────────
    ("sound", [r"^SND", r"^SOUND$"]),

    # ── input / hid ──────────────────────────────────────────────────────
    ("input", [r"^INPUT", r"^KEYBOARD", r"^MOUSE", r"^TOUCHSCREEN", r"^JOYSTICK", r"^HID",
               r"^SERIO", r"^EVDEV", r"^GAMEPORT"]),

    # ── networking ───────────────────────────────────────────────────────
    ("bluetooth", [r"^BT($|_)"]),
    ("wireless", [r"^CFG80211", r"^MAC80211", r"^IEEE80211", r"^RFKILL", r"^WIRELESS_EXT"]),
    ("wifi-driver", [r"^IWLWIFI", r"^IWLMVM", r"^IWLDVM", r"^ATH(5|6|9|10|11|12)K", r"^RTL8",
                     r"^RT2X", r"^BRCM", r"^MWIFIEX", r"^MT76", r"^MT(79)?21"]),
    ("eth-driver", [r"^R8169$", r"^R8152$", r"^E1000", r"^IGB($|_)", r"^IGC", r"^IXGBE", r"^TG3",
                    r"^BNX2", r"^MLX", r"^I40E", r"^ICE($|_)", r"^ENA$", r"^FM10K"]),
    ("network", [r"^NET($|_)", r"^INET$", r"^INET6$", r"^IP_", r"^IPV6", r"^TCP_", r"^UDP_",
                 r"^NETFILTER", r"^NF_", r"^NETLINK", r"^BRIDGE", r"^VLAN_", r"^BONDING", r"^VETH",
                 r"^TUN$", r"^WIREGUARD", r"^OPENVSWITCH", r"^GENEVE", r"^VXLAN", r"^MACVLAN",
                 r"^MACVTAP", r"^IPVLAN", r"^MACSEC", r"^L2TP", r"^PPP", r"^SLIP", r"^MAC_PARTITION$",
                 r"^PHYLIB$", r"^MDIO", r"^MII$", r"^DUMMY$"]),

    # ── filesystems ──────────────────────────────────────────────────────
    ("fs", [r"_FS$", r"^FUSE", r"^OVERLAY", r"^SQUASHFS", r"^NFS", r"^CIFS", r"^CEPH",
            r"^BTRFS", r"^EXT[234]", r"^XFS", r"^F2FS", r"^NTFS", r"^EXFAT", r"^ISO9660",
            r"^UDF", r"^MSDOS", r"^VFAT", r"^FAT_FS$", r"^AUTOFS", r"^9P_FS", r"^ZFS",
            r"^EROFS", r"^BCACHEFS", r"^GFS2", r"^OCFS2", r"^DLM$", r"^QUOTA", r"^FANOTIFY",
            r"^INOTIFY", r"^DNOTIFY", r"^FSCACHE", r"^CACHEFILES"]),
    ("binfmt", [r"^BINFMT"]),

    # ── storage / block ──────────────────────────────────────────────────
    ("nvme", [r"^NVME", r"^BLK_DEV_NVME"]),
    ("scsi", [r"^SCSI", r"^SATA", r"^PATA", r"^ATA($|_)"]),
    ("block", [r"^BLK_DEV", r"^DM_", r"^MD_", r"^BCACHE($|_)", r"^IOSCHED", r"^MQ_IOSCHED"]),
    ("mmc", [r"^MMC"]),
    ("mtd", [r"^MTD"]),
    ...
    # ── platform-specific ────────────────────────────────────────────────
    ("platform-x86", [r"^DELL", r"^X86_PLATFORM", r"^THINKPAD", r"^IDEAPAD", r"^ASUS", r"^ACER",
                       r"^HP_WMI", r"^HP_ACCEL", r"^MSI_LAPTOP", r"^TOSHIBA", r"^FUJITSU",
                       r"^INTEL_(MENU|HID|VBTN|WMI|RST|ATOM|PUNIT|TURBO|UNCORE|TELEMETRY|PMC|PMT|ISH|IPS|SCU|SOC|VSEC|SPEED|SDSI)",
                       r"^AMD_PMC", r"^AMD_HSMP", r"^AMD_PMF"]),

    # ── thermal / power-supply / regulator / leds / watchdog / rtc ───────
    ("thermal", [r"^THERMAL", r"^INT3400_THERMAL", r"^INT340X_THERMAL", r"^X86_PKG_TEMP_THERMAL"]),
    ...
    ("firmware", [r"^FW_LOADER", r"^EXTRA_FIRMWARE", r"^FIRMWARE_", r"^EFI_VARS_PSTORE",
                  r"^TCG_", r"^TPM_"]),
    ("mei", [r"^INTEL_MEI"]),
    ("cxl", [r"^CXL"]),
    ("edac", [r"^EDAC"]),
